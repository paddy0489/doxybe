...
def outputDebugCallbackIfNeeded(inFunction):
    ...
    def __wrappedFunction(*args, **kwargs):
        ...
        if options.debug:
            print("# CALLBACK: %s" % inFunction.__name__, file=sys.stderr)
...
class Doxypy(object):
    def __init__(self):	
        ...
        self.importline_re = re.compile("^\s*(import)")

        self.multiline_defclass_start_re = re.compile("^(\s*)(def)(\s.*)?[\s|,]$")
    ...
    def startCommentSearch(self, match):
        ...
        post = ""
        if ((from_state == "DEFCLASS_MULTI" or from_state == "FILEHEAD" or from_state == "DEFCLASS_BODY") and to_state == "DEFCLASS"):
            post = ":"

        if ":" in self.fsm.current_input:
            self.fsm.current_input= self.fsm.current_input.replace(":","(")
            post = ")" + post

        self.defclass = [self.fsm.current_input + post]
        self.comment = []
    ...
    def appendCommentLine(self, match):
        ...
        or (from_state == "FILEHEAD" and to_state == "FILEHEAD"):
            ...
            activeCommentDelimStart = match.group(1)
            activeCommentDelimEnd = match.group(2)
            line = self.fsm.current_input
            self.comment.append(line[line.find(activeCommentDelimStart)+len(activeCommentDelimStart):line.rfind(activeCommentDelimEnd)].rstrip())
    ...
    def appendNormalLine(self, match):
        ...
        self.output.append(self.fsm.current_input.replace("end", ""))
    ...
    def appendDefclassLine(self, match):
        ...
        self.defclass.append(self.fsm.current_input + post)
